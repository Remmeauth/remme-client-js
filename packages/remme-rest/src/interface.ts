...
export interface IRemmeRest {
    nodeAddress(): string;
    sslMode(): boolean;
    // getRequest<Output>(method: RemmeMethods | ValidatorMethods, urlParams?: string, queryParams?: IQueryParams)
    //     : Promise<Output>;
    getRequest<Output>(method: RemmeMethods | ValidatorMethods, requestPayload?: IQueryParams)
        : Promise<Output>;
